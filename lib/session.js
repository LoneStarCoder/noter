...
// Stateless signed session cookie holding what this browser has unlocked:
//   { p: { page: fingerprint }, f: fingerprint, a: fingerprint, e: expiresAt }
// Fingerprints tie each grant to the current password, so changing a password
...
function createSessions(secret) {
  ...
  function read(req) {
    ...
    try {
      ...
      return { p: pages, f: typeof data.f === 'string' ? data.f : null, a: typeof data.a === 'string' ? data.a : null };
    } catch (err) {
      return empty;
    }
  }
  ...
  function write(req, res, session) {
    const hasGrants = Object.keys(session.p).length > 0 || session.f || session.a;
    const secure = req.secure ? '; Secure' : '';
    if (!hasGrants) {
      res.append('Set-Cookie', `${COOKIE_NAME}=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0${secure}`);
      return;
    }
    ...
    res.append('Set-Cookie',
      `${COOKIE_NAME}=${value}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${MAX_AGE_MS / 1000}${secure}`);
  }
